...
export const onSubmit = async (formData: { krs: FileList }) => {
  ...
  const currentDataSemester = parseInt(
    rawSemesterIndicator.replace("Daftar Rencana Studi Semester : ", "").trim()
  );
  ...
  const restructureData = DAYTIME_REF.map((daytime) => {
    const currentDaySubjects = tempGreedArray
      ...
      .sort(
        (a, b) => timeToNumber(a.time.startedAt) - timeToNumber(b.time.endedAt)
      )
      .map((s) => ({
        ...s,
        time: {
  ...
  console.log(validateRestructuredData);
};
